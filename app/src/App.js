import { jsx as _jsx, jsxs as _jsxs } from "react/jsx-runtime";
import { useEffect, useState, useMemo } from "react";
import { useLocation } from "react-router-dom";
import Providers from "@/components/Providers";
import { isAddress } from "viem";
import { ConnectButton } from "@rainbow-me/rainbowkit";
import SwapWidget from "@ensofinance/shortcuts-widget";
import logoUrl from "./logo_black_white.png";
import "@rainbow-me/rainbowkit/styles.css";
import "./App.css";
const loadConfig = async () => {
    try {
        const response = await fetch("./config.json");
        if (!response.ok)
            throw new Error("Failed to load config.json");
        return await response.json();
    }
    catch (error) {
        console.error("Error loading config.json:", error);
        return {};
    }
};
const EnsoApiKey = import.meta.env.VITE_ENSO_API_KEY;
function App() {
    const [config, setConfig] = useState(null);
    const location = useLocation();
    const props = useMemo(() => {
        const searchParams = new URLSearchParams(window.location.search);
        const tokenInParam = searchParams.get("tokenIn");
        const tokenOutParam = searchParams.get("tokenOut");
        const chainIdParam = searchParams.get("chainId");
        const props = {
            apiKey: EnsoApiKey,
        };
        if (chainIdParam) {
            props.chainId = parseInt(chainIdParam);
            if (isAddress(tokenInParam))
                props.tokenIn = tokenInParam;
            if (isAddress(tokenOutParam))
                props.tokenOut = tokenOutParam;
        }
        return props;
    }, [location]);
    useEffect(() => {
        loadConfig().then(setConfig);
        // // Set the title of the page from the environment variable
        // if (import.meta.env.VITE_APP_TITLE) {
        //   document.title = `ENSO | ${import.meta.env.VITE_APP_TITLE}`;
        // }
        // console.log(`\n\x1b[41m config ==> `, config, `\x1b[0m\n`)
        //
        // // Set the favicon of the page from the environment variable
        // if (import.meta.env.VITE_APP_LOGO_URL) {
        //   const favicon = document.querySelector("link[rel='icon']");
        //   if (favicon instanceof HTMLLinkElement) {
        //     favicon.href = import.meta.env.VITE_APP_LOGO_URL;
        //   }
        // }
    }, []);
    // if (!config) return <div>Loading...</div>;
    // console.log(`\n\x1b[41m config ==> `, config, `\x1b[0m\n`)
    // Set the title of the page from the environment variable
    if (config?.VITE_APP_TITLE) {
        console.log(`\n\x1b[41m config.VITE_APP_TITLE ==> `, config.VITE_APP_TITLE, `\x1b[0m\n`);
        document.title = `ENSO | ${config.VITE_APP_TITLE}`;
    }
    // Set the favicon of the page from the environment variable
    if (config?.VITE_APP_LOGO_URL) {
        const favicon = document.querySelector("link[rel='icon']");
        if (favicon instanceof HTMLLinkElement) {
            favicon.href = config.VITE_APP_LOGO_URL;
        }
    }
    return (_jsx(Providers, { children: _jsxs("div", { style: {
                flexDirection: "column",
                display: "flex",
                justifyContent: "center",
                height: "100%",
                width: "100%",
                gap: "10px",
            }, children: [_jsxs("div", { style: {
                        position: "absolute",
                        top: "1%",
                        left: "0%",
                        display: "flex",
                        width: "100%",
                        justifyContent: "space-around",
                    }, children: [_jsx("img", { src: logoUrl, alt: "Enso", style: { height: "50px" } }), _jsx(ConnectButton, {})] }), _jsx(SwapWidget, { ...props, enableShare: true, indicateRoute: true, adaptive: true })] }) }));
}
export default App;
